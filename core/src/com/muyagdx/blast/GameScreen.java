...
public class GameScreen implements Screen {
	...
	public GameScreen(final Blast gam, int selectedMode){//TODO move selectemode to Blast CLASS
		this.game = gam;
		isPerfect = true;
		touchControl = 0;
		worldCoords = new Vector3(0, 0, 0);
		...
		background = new Stage(selectedMode);
		background.setStage(game.getCurrentStage());
		if(game.getCurrentStage() != 0)
			background.setBoss(1);
		...
		background.setRect(new Rectangle(0,0,1800,2000)); // TODO make x and y and width and height variable of each stage's dimensions
		camera = new OrthographicCamera();
		camera.setToOrtho(false, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());		//TODO get screen dimensions
		camera.position.set(camera.viewportWidth / 2f, camera.viewportHeight / 2f, 0);
		pause.setImage("pausebutton.png");
		player.setRect(new Rectangle(Gdx.graphics.getWidth()/2-32,Gdx.graphics.getHeight()/2-32,64,64));
...
//TODO Change action flow to be free movement and camera movement, fighting small random enemies 
//in a maze until destination is reached then call gamebossscreen if stage has boss
	public void renderAction(float delta){
		Gdx.gl.glClearColor(0.3f, 0.35f, 0.2f, 0.3f);
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
		touchPos = new Vector3();
		touchPos.set(Gdx.graphics.getWidth()/2, Gdx.graphics.getHeight()/2, 0);
		touchListener();
		keyListener();
        camera.update();
		game.batch.setProjectionMatrix(camera.combined);
		...
		//if(player.getX() > Gdx.graphics.getWidth() - 64) player.setX(Gdx.graphics.getWidth() - 64);
...
//		moveOpponentWeapon();
	...
	private void touchListener(){
		if(Gdx.input.isTouched()) { //TODO stop the extra effect of touch 
			...
			if(pause.hit(new Rectangle(touchPos.x - worldCoords.x, touchPos.y - worldCoords.y, 1, 1))){
				...
				//TODO Move camera and player objects
				if(touchPos.x > player.getX()+player.getRect().width){
					if(camera.position.x < background.getRect().getWidth() - Gdx.graphics.getWidth()/2 && player.getX() > Gdx.graphics.getWidth()/2){
						camera.translate(1,0);
						worldCoords.set(worldCoords.x+1, worldCoords.y, 0);
					}
					player.setDirectionX(-1);
				}
				else if(touchPos.x < player.getX()){
					if(camera.position.x > Gdx.graphics.getWidth()/2 && player.getX() < background.getRect().getWidth() - Gdx.graphics.getWidth()/2){
					camera.translate(-1,0);
					worldCoords.set(worldCoords.x-1, worldCoords.y, 0);
				}
					player.setDirectionX(1);
				...
				if(touchPos.y > player.getY()+player.getRect().height){
					if(camera.position.y < background.getRect().getHeight() - Gdx.graphics.getHeight()/2 && player.getY() > Gdx.graphics.getHeight()/2){
						camera.translate(0,1);
						worldCoords.set(worldCoords.x, worldCoords.y+1, 0);
					}
					player.setDirectionY(-1);
				}
				else if(touchPos.y < player.getY()){
					if(camera.position.y > Gdx.graphics.getHeight()/2 && player.getY() < background.getRect().getHeight() - Gdx.graphics.getHeight()/2){
						camera.translate(0,-1);
						worldCoords.set(worldCoords.x, worldCoords.y-1, 0);
					}
					player.setDirectionY(1);
				...
				player.autoMove1WayX((int) touchPos.x, player.getSpeed());
				player.autoMove1WayY((int) touchPos.y, player.getSpeed());
...
//			    camera.position.y = MathUtils.clamp(camera.position.y, effectiveViewportHeight / 2f, 100 - effectiveViewportHeight / 2f);
		...
		//TODO make different fight mechanics for normal fights. maybe RPG style
...
//			}
	...
	private void keyListener(){
		...
		if(Gdx.input.isKeyPressed(Keys.Z)){
			player.move(-200 * Gdx.graphics.getDeltaTime() - player.getSpeed(), 0) ;
			camera.translate(-1, 0);
		}
...
//	}
	...
	public void renderPaused(float delta){
		...
		game.batch.draw(blueCircle.getImage(), 745 + worldCoords.x, 230 + worldCoords.y);
		game.batch.draw(blueCircle.getImage(), 8 + worldCoords.x, 230 + worldCoords.y);
		game.font.draw(game.batch, String.valueOf(bonusCollected), 750 + worldCoords.x, 270 + worldCoords.y);
		game.font.draw(game.batch, String.valueOf(specialsGathered), 12 + worldCoords.x, 270 + worldCoords.y);
		game.font.draw(game.batch, "Waiting for your promised return!", 250 + worldCoords.x, 340 + worldCoords.y);
		//TODO change options to menu items with text
		game.batch.draw(resume.getImage(), resume.getX() + worldCoords.x, resume.getY() + worldCoords.y);  
		game.batch.draw(quit.getImage(), quit.getX() + worldCoords.x, quit.getY() + worldCoords.y);
		game.batch.end();
		...
		if(Gdx.input.isTouched()) {
			...
			if (resume.getRect().contains(new Rectangle(touchPos.x - worldCoords.x,touchPos.y - worldCoords.y,1,1))){
				game.setState(ACTION);
			}
			else if (quit.getRect().contains(new Rectangle(touchPos.x - worldCoords.x,touchPos.y - worldCoords.y,1,1)))
			{
	...
	public void renderBG(){
		...
		game.batch.begin();
		game.batch.draw(background.getImage(), background.getX(), background.getY(), 1800, 2000);
		game.batch.end();
	}
	...
	private void renderPlayer(){
		...
		// TODO change drawing to keep player midscreen until background borders
	...
	private void renderScore(){
		...
		game.batch.enableBlending();
		game.font.setColor(0.9f, 0.5f, 0.5f, 1);
		game.batch.draw(blueCircle.getImage(),745 + worldCoords.x, 430 + worldCoords.y);
		game.batch.draw(blueCircle.getImage(),5 + worldCoords.x, 430 + worldCoords.y);
		//game.batch.draw(lifeBar, 20 + worldCoords.x, 450 + worldCoords.y, opponent.getLife(), 20);
		game.batch.draw(lifeBar, 800-player.getLife() + worldCoords.x, 450 + worldCoords.y, player.getLife(), 20);
		game.batch.draw(pause.getImage(), pause.getX() + worldCoords.x, pause.getY() + worldCoords.y);
		game.font.draw(game.batch, String.valueOf(bonusCollected), 750 + worldCoords.x, 470 + worldCoords.y); //TODO replace by speed bar, power bar and score
		game.font.draw(game.batch, String.valueOf(specialsGathered), 12 + worldCoords.x, 470 + worldCoords.y);  
		game.batch.end();
	}
